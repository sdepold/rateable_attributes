...
MakeRateable is a re-implementation of ActsAsRateable for multiple attribute support.
...
In order to create the rating table just run the following command:
  ...
  script/generate make_rateable
  
This will generate a migration. Run rake db:migrate.

=== In the model
  
  make_rateable :rateable_attributes => [:graphics, :sound, :whatever]
